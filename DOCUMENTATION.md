...
## Current Status — AI Coach three modes (task in progress)

**Done (mobile):** `CoachEndpoint` + `coachEndpointProvider` (mode resolution, private-mode-first), proxy transport, per-send JWT, host-aware preflight, per-mode error mapping, ungated entry point, transport-aware setup gate, `ai.coachModes.*` i18n in 5 locales, "unlimited" copy removed from both apps, BYOK copy reframed in 5 locales + "Active engine" settings row, dead `generateResponse` removed.

**Done (desktop):** third `CoachBackendKind.standard` + `StandardCoachBackend`, name-based `fromCode`, Standard as the default engine, `effectiveCoachBackend` + `resolveStandardCoachStatus` (private-mode-first), per-send JWT via an injectable `authorization` closure, injectable `errorMapper` keyed on the function's `error.code`, 45s cold-start budget, 3-way picker in `coach_settings_dialog.dart`, 3-way chip + settings row, BYOK copy reframed in 5 locales.

**Immediate next step — re-order `pro_features_modal` in both apps.** The coach is now partly free, so the Pro pitch has to change: Pro buys **"no setup"**, not "access". The modal still leads with the coach as though it were the headline unlock, which is both inaccurate (BYOK is free) and the 3.1.2 accuracy risk we are already rejected under. Files: `mobile/lib/ui/widgets/pro_features_modal.dart`, desktop's equivalent (`desktop/test/pro_features_modal_test.dart` exists, so it has one).

**Then — task #9, the 5.1.2(i) consent.** Needs its OWN consent for the proxy path, distinct from BYOK: there the user is not choosing the recipient, we are. Name OpenRouter, Inc. and Google LLC via Google Cloud Vertex AI; standalone, un-pre-ticked, non-bundled, BEFORE first transmission; declining must leave the app fully usable; do NOT fire ATT. Mobile keys are `ai.privateConsentTitle` / `ai.privateConsentBody` (NOT desktop's `privateAi.consentBody`). **Note `_ensurePrivateAiConsent` returns true immediately in Supabase mode (`ai_chat_screen.dart:257`), so cloud users are currently asked nothing** — that is the gap.

**Task #7 (Screen Time / 2.1) is untouched** and blocked first on the accumulation-reset bug at `DOCUMENTATION.md:920`. Cannot be verified here at all: no iOS SDK, and FamilyControls does not work in the Simulator.
