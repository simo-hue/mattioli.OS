...
**Done (mobile):** `CoachEndpoint` + `coachEndpointProvider` (mode resolution, private-mode-first), proxy transport, per-send JWT, host-aware preflight, per-mode error mapping, ungated entry point, transport-aware setup gate, `ai.coachModes.*` i18n in 5 locales, "unlimited" copy removed from both apps.

**Done (desktop):** third `CoachBackendKind.standard` + `StandardCoachBackend`, name-based `fromCode`, Standard as the default engine, `effectiveCoachBackend` + `resolveStandardCoachStatus` (private-mode-first), per-send JWT via an injectable `authorization` closure, injectable `errorMapper` keyed on the function's `error.code`, 45s cold-start budget, 3-way picker in `coach_settings_dialog.dart`, 3-way chip + settings row, BYOK copy reframed as "Connect your OpenRouter account" in all 5 locales.

**Immediate next step — the mobile copy reframe.** Desktop's `ai.apiKey.*` now reads as *an account you connect*; mobile's still reads as *a key that unlocks*, and mobile is the client Apple actually rejected. Mirror the desktop strings across mobile's 5 locales.

**Then:** re-order `pro_features_modal` in both apps now that the coach is partly free (Pro buys "no setup", not "access"); then task #9, the 5.1.2(i) consent, which needs its own consent for the proxy path distinct from BYOK — the user is not choosing the recipient there, we are — naming OpenRouter, Inc. and Google LLC via Google Cloud Vertex AI, before first transmission, un-pre-ticked, with the app fully usable if declined. Note `_ensurePrivateAiConsent` returns true immediately in Supabase mode (`ai_chat_screen.dart:257`), so cloud users are currently asked nothing. Task #7 (Screen Time / 2.1) is untouched and blocked on the accumulation-reset bug at `DOCUMENTATION.md:920`.
