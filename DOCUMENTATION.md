...
**Immediate next step — desktop's third backend.** It is far smaller than the 1–2 days estimated, because `supabase/functions/ai-coach` is **OpenAI-compatible by design**: it reads `messages`, ignores `model`/`temperature`, and pipes OpenRouter's SSE straight back. So `OpenAiCompatibleClient` can talk to it unchanged — the work is a third `CoachBackend` enum value (mind the binary `fromCode` parse at `coach_backend.dart:16-19`), a `baseUrl` of `<supabase>/functions/v1/ai-coach`, an Authorization header resolved **per request** from the live session (the transport captures headers at construction today — that is the trap), and a widened `firstTokenTimeout` past 20s for Edge Function cold starts (`cloud_coach_backend.dart:28-30`).

**Then:** reframe the BYOK settings copy as "Connect your OpenRouter account" (`ai.apiKey.*`, both apps) — the framing, not the mechanism, is what Apple pattern-matched to a licence-key gate; re-order `pro_features_modal` now that the coach is partly free; desktop's 3-way backend picker in `coach_settings_dialog.dart`; and task #9, the 5.1.2(i) consent, which needs the proxy's pinned provider list to be truthful.
