...
## Current Status — AI Coach three modes (task #5 COMPLETE)

**Mobile:** `CoachEndpoint` + `coachEndpointProvider` (mode resolution, private-mode-first), proxy transport, per-send JWT, host-aware preflight, per-mode error mapping, ungated entry point, transport-aware setup gate, `ai.coachModes.*` i18n in 5 locales, BYOK copy reframed + "Active engine" settings row, dead `generateResponse` removed, Pro pitch re-ordered on both surfaces.

**Desktop:** third `CoachBackendKind.standard` + `StandardCoachBackend`, name-based `fromCode`, Standard as the default engine, `effectiveCoachBackend` + `resolveStandardCoachStatus` (private-mode-first), per-send JWT via an injectable `authorization` closure, injectable `errorMapper` keyed on the function's `error.code`, 45s cold-start budget, 3-way picker + chip + settings row, BYOK copy reframed in 5 locales, Pro pitch re-ordered.

**Guideline 3.1.1 is now fixed end to end** — with the caveat that **none of it has run against the live Edge Function**: it is not deployed and `OPENROUTER_API_KEY` is not set. Everything is verified against mocked transports only. The proxy path is unproven in reality until Simone applies the migration, sets the secret, deploys, and makes one live request (see TO_SIMO_DO.md — the provider pin in particular is asserted at runtime but has never been observed).

**Immediate next step — task #9, the 5.1.2(i) consent.** The proxy path needs its OWN consent, distinct from BYOK: there the user is not choosing the recipient, we are. Name **OpenRouter, Inc.** and **Google LLC via Google Cloud Vertex AI**; standalone, un-pre-ticked, non-bundled, BEFORE first transmission; declining must leave the app fully usable; do NOT fire ATT. Mobile keys are `ai.privateConsentTitle` / `ai.privateConsentBody` (NOT desktop's `privateAi.consentBody`). **The gap: `_ensurePrivateAiConsent` returns true immediately in Supabase mode (`ai_chat_screen.dart:257`), so cloud users are asked nothing today.** The privacy policy must also satisfy 5.1.1(i): equal protection, retention/deletion, revocation.

**Then task #7 (Screen Time / 2.1)**, untouched. Blocked first on the accumulation-reset bug at `DOCUMENTATION.md:920` (`runVerificationReconcile` → `syncMonitoredGoals` on every foreground; `AppDelegate.swift:624` stops and re-adds monitoring, likely resetting DeviceActivity threshold accumulation mid-interval). Then: localise the extension notification (`DeviceActivityMonitorExtension.swift:49-50`, hardcoded English), add `NSFamilyControlsUsageDescription`, flip `VerificationConfig.screenTimeEnabled = true`, clear the stale "UNVERIFIED/compile-pending" comments. Mind Apple's 20-activity cap. **Cannot be verified here at all**: no iOS SDK, and FamilyControls does not work in the Simulator.
