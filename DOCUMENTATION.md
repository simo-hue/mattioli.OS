...
- [2026-07-15]: Add Confirmation Dialog on Goal Deletion
  - *Details*: Added a confirmation dialog using `EvolveAlertDialog` when deleting a goal from the goals page in the desktop app. This prevents accidental deletion of goals.
  - *Tech Notes*: Reused the existing translation keys `t.palette.deleteGoalTitle` and `deleteGoalMessage` for the dialog text, and applied a destructive style for the "Delete" action to match the existing Command Palette behavior. Handled in `_GoalItemState` within `goals_page.dart`.
