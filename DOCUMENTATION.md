...
- **2026-07-17: Set default calendar view to monthly on desktop**
  - *Details*: Modified the default value of the calendar view setting to 'mese' (monthly) on desktop platforms (macOS, Windows, Linux) while preserving 'settimana' (weekly) for mobile.
  - *Tech Notes*:
    - Imported `dart:io` in `mobile/lib/providers/settings_provider.dart`.
    - Updated `_defaultSettings`, `_loadFromPrefs`, and `_settingsFromPrivateRow` to return `'mese'` when `Platform.isMacOS || Platform.isWindows || Platform.isLinux` is true, keeping `'settimana'` as the fallback otherwise.
