...
## 🍏 5. iOS App Store Guideline 4.10 Compliance (Monetizing Device Capabilities)
Abbiamo completamente risolto la violazione della linea guida 4.10 rendendo la funzionalità di **Blocco Biometrico (Face ID / Touch ID)** del tutto **GRATUITA** per tutti gli utenti, rimuovendo qualsiasi blocco a pagamento ("isPro").
1. Abbiamo rimosso la dicitura della "Sicurezza Biometrica Avanzata" / "Protezione Biometrica" dal paywall dell'abbonamento e dal modal delle features premium, sostituendola con l'ottimo benefit "Abitudini Illimitate".
2. Il Blocco Biometrico è ora accessibile a tutti gli utenti nelle impostazioni di Privacy e Sicurezza.
3. **Note per la Review (Review Notes)**: Aggiungi anche questo paragrafo nelle note di sottomissione per guidare i reviewer:
   ```text
   Regarding Guideline 4.10 (Monetizing Built-In Capabilities):
   We have completely revised the monetization structure of Evolve. Biometric authentication (Face ID and Touch ID) is now 100% free and accessible to all users under the Privacy and Security settings, without requiring any premium subscription or locked states.
   Any references to biometric security as a paid feature have been entirely removed from our subscription page and premium onboarding dialogs.
   ```

## 🍏 6. iOS App Store Guideline 2.1(b) & 2.2 Compliance (In-App Purchases Submission)
La notifica di "App Completeness" e "Beta Testing" (Guideline 2.1(b) e 2.2) era dovuta al fatto che l'app presentava riferimenti agli abbonamenti ma i prodotti (mensile/annuale) non erano ancora stati associati e inviati per la review su App Store Connect, rendendo impossibile per i reviewer sottoscrivere l'abbonamento (e rendendo quindi le funzioni pro inutilizzabili).
1. Accedi ad **App Store Connect** > seleziona la tua app > vai alla sezione **Abbonamenti** (Subscriptions) nel menu a sinistra.
2. Assicurati che i prodotti (Monthly e Annual) creati nel tuo gruppo di abbonamento abbiano caricato uno **screenshot della review** (obbligatorio per Apple!) e siano stati **inviati per la verifica** (Submitted for Review) insieme alla nuova build della tua app.
3. Una volta che i prodotti sono associati alla build inviata, il paywall si caricherà con successo e l'app sarà perfettamente testabile in ogni sua parte dai reviewer, risolvendo istantaneamente sia la Guideline 2.1(b) che la 2.2.
