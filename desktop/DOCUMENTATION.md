...
- **WS6** — route private settings through the DB `profiles` row (theme/accent/calendar/language/24h/haptics/notif flags/times/biometric) instead of `SharedPreferences`, for privacy + Phase-2 sync readiness. Larger refactor; private settings currently persist in SharedPreferences (they DO survive restarts, just not encrypted/sync-ready).
- **WS7** — stand up **slang**; full desktop localization (en/it/es/de/ar) incl. the new Private-Mode strings (AI consent dialog in `ai_coach_page.dart`, delete/entitlement snackbars, name dialog) + Arabic **RTL pass** (port mobile `lib/core/rtl.dart`). Large, app-wide effort.
- **WS9** — guard tests: no-Supabase-call-in-Private (needs the providers refactored to accept an injectable store), settings-separation, notification-action routing. The 37 existing Private-Mode tests already prove the DB/analytics logic runs with zero Supabase dependency.
