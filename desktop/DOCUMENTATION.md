...
- [2026-07-30]: macOS Release archive — kill the `objective_c1.framework` warning by dropping the FFI path_provider
  ...
  - *Not verified here*: no Xcode and no CocoaPods on this machine, so `desktop/macos/Podfile.lock` still lacks the `path_provider_foundation (0.0.1)` pod and the archive itself was not re-run. The mac mini must `pod install` before the next Archive or Xcode fails to compile the regenerated registrant (*no such module 'path_provider_foundation'*) — steps logged in `TO_SIMO_DO.md`. That the warning is gone can only be confirmed from a real archive log.
