...
The desktop (macOS) app is broadly at parity with mobile at the reasoning/formula level — streak logic is byte-identical, private analytics formulas are faithfully ported, and the core sync engine is shared-package-by-construction. At audit time the two clients diverged materially at the edges in ways that caused real data loss, privacy leaks, and monetization holes. **That has since been remediated: read the rest of this document as the 2026-07-14 audit snapshot it is, not as a to-do list.** The 2026-07-14 parity port (see `desktop/DOCUMENTATION.md`) closed 30 of the 32 findings below, leaving only #9 (a MOBILE-side fix; desktop was already correct) and #28 — and #28 itself was closed on 2026-07-27 when `toggleHabitForDay` began refusing verified habits. Spot-checked 2026-07-27: `goal_logs.value` survives import and export (`desktop/lib/core/desktop_backup_import_service.dart:328`, `desktop/lib/core/desktop_private_db.dart:501,1329,1346`), `PrivacyUtils` scrubs messages and extras before Sentry (`desktop/lib/core/privacy_utils.dart`, `desktop/lib/core/app_logger.dart:254,280,289,304`), the 5-habit and per-habit-stats Pro gates are enforced (`desktop/lib/features/dashboard/application/dashboard_controller.dart:348`, `desktop/lib/features/statistics/presentation/statistics_page.dart:270`), and the placeholder Sentry DSN is treated as unconfigured (`desktop/lib/core/desktop_sentry_service.dart:30-33`).
