...
/// Schema + migrations for the Private-Mode local database.
///
/// Ported verbatim from the mobile client (`mattioli_os`) so the private schema
/// is a single source of truth across clients: identical tables, columns,
/// constraints and indexes. This keeps the ported analytics numerically exact
/// and keeps eventual iCloud-sync records format-compatible between iOS/macOS.
///
/// Extracted from the store so the DDL and migration logic can run against both
/// the production SQLCipher database AND an in-memory `sqflite_common_ffi`
/// database in tests (encryption is orthogonal to schema correctness).
/// Everything here operates on a [DatabaseExecutor], so it works inside
/// transactions too.
