...
/// `PositionedDirectional`) and the app's `Directionality` follows the active
/// locale (the desktop `MaterialApp` supplies `ar` via `supportedLocales` +
/// `GlobalMaterialLocalizations.delegates`, so `ar` lays out right-to-left).
/// Glyph-based icons, however, are not mirrored automatically — a
/// `chevronLeft` "back" affordance still points left under RTL unless we swap
/// it. [directionalIcon] / [DirectionalIcon] make that swap explicit at the
/// call site. Ported verbatim from mobile `lib/core/rtl.dart`.
...
class DirectionalIcon extends StatelessWidget {
  ...
  const DirectionalIcon(this.ltr, this.rtl, {this.color, this.size, super.key});
