...
enum PrivateRecoveryStatus {
  ...
  /// OPPOSITE: the ciphertext is intact and its key exists somewhere, so the app
  /// must not stash, re-pull, reset or delete anything on its own. It reports,
  /// explains, and waits. This is the state the macOS incident actually
  /// produced, and the old code had no name for it — so it fell into [error]
  /// and was offered a one-tap delete.
  undecryptable,

  /// The stored schema is NEWER than this build understands: the user opened an
  /// older build (a TestFlight "previous build", a kept `.app`). The data is
  /// intact and correctly keyed; the only remedy is to reopen the newer build.
  /// Never destructive.
  schemaTooNew,
...
class PrivateRecoveryResult {
  ...
  /// diagnostic code shown on the failure screen and copied into the
  /// diagnostics bundle — and, critically, whether a destructive action may be
  /// offered at all ([allowsDestructiveRecovery]).
  final PrivateDbOpenFailure? failure;

  /// Whether the UI may offer the (now non-destructive, but still
  /// data-displacing) reset for this result. Deliberately computed here rather
  /// than in the widget, so both apps and every test share ONE definition of
  /// "is it acceptable to move this user's database out from under them".
...
Future<PrivateRecoveryResult> openOrRecoverPrivate(
  ...
  try {
    ...
    // destructive recovery: only genuine corruption earns that, and everything
    // else lands in a state that explains itself and leaves the file alone.
