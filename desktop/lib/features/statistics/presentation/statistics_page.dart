...
class _StatisticsPageState extends ConsumerState<StatisticsPage> {
  ...
  void _moveTab(int direction) {
    if (_scope == _AnalyticsScope.global) {
      final values = _GlobalTab.values;
      final currentIndex = values.indexOf(_globalTab);
      final newIndex = currentIndex + direction;
      if (newIndex >= 0 && newIndex < values.length) {
        setState(() => _globalTab = values[newIndex]);
      }
    } else {
      final values = _HabitTab.values;
      final currentIndex = values.indexOf(_habitTab);
      final newIndex = currentIndex + direction;
      if (newIndex >= 0 && newIndex < values.length) {
        setState(() => _habitTab = values[newIndex]);
      }
    }
  }
