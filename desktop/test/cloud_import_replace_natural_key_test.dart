...
//
// Desktop twin of mobile/test/cloud_import_replace_natural_key_test.dart — the
// two service files must not drift.
import 'dart:convert';
...
void main() {
  ...
  // ── Pure-plan twins (no network): the decisions the service depends on ─────

  CloudImportPlan replacePlan({
    List<Map<String, dynamic>> cats = const [],
    List<Map<String, dynamic>> goals = const [],
    List<Map<String, dynamic>> macros = const [],
    List<Map<String, dynamic>> logs = const [],
  ...
  test('REPLACE: a backup row that LOSES last-write-wins is still written', () {
    ...
    final p = replacePlan(
      goals: [
        {'id': 'g1', 'title': 'G', 'color': '#1', 'start_date': '2026-01-01', 'updated_at': t},
      ],
      macros: [
        {'id': 'm1', 'title': 'Fit', 'status': 'active', 'type': 'annual', 'updated_at': t},
      ],
      logs: [
        {'id': 'l1', 'goal_id': 'g1', 'date': '2026-01-01', 'status': 'done', 'updated_at': t},
      ],
      moods: [
        {'id': 'd1', 'date': '2026-01-01', 'mood_score': 5, 'energy_score': 5, 'updated_at': t},
      ],
      existingGoals: const {'g1': t},
      existingMacros: const {'m1': t},
      existingLogs: const {
        'g1|2026-01-01': {'id': 'l1', 'goal_id': 'g1', 'date': '2026-01-01', 'updated_at': t},
      },
      existingMoods: const {
        '2026-01-01': {'id': 'd1', 'date': '2026-01-01', 'updated_at': t},
      },
    );
  ...
  test('REPLACE: a log for a server-only goal (absent from the backup) is not '
    ...
    final p = replacePlan(
      logs: [
        {'id': 'l1', 'goal_id': 'gone', 'date': '2026-01-01', 'status': 'done', 'updated_at': '2026-06-01T00:00:00.000Z'},
      ],
      existingGoals: const {'gone': '2020-01-01T00:00:00.000Z'},
    );
    expect(p.logs, isEmpty);
  });

  test('REPLACE: case-variant duplicate names plan ONE row per final id', () {
    // macro_goal_categories UNIQUE(user_id,name) is case-sensitive, so an
    // account can hold both 'Work' and 'work', while reconciliation matches on
    // trim+lowercase. Emitting the same final id twice in one `on_conflict=id`
    // upsert is Postgres 21000 ("ON CONFLICT DO UPDATE command cannot affect
    // row a second time") and aborts the whole Replace import on its FIRST
    // table. reconcileCategoriesByName documents this dedup as a guarantee.
    final p = replacePlan(
      cats: [
