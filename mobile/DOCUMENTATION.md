...
## [2026-09-12 14:40]: Feature — Edit any past day in the habit day sheet (Edit → Save)
*Details*: The day sheet (`DayDetailsModal`, opened from the habits calendar and the weekly radar) used to refuse anything older than yesterday with "You can only edit today and yesterday!". Every past day is now editable, but deliberately: today and yesterday keep the one-tap check-in (tap a card to cycle none → done → missed → none), while an older day opens in view mode with a pencil beside the X. Tapping a card there shows "Tap Edit to change this day" and writes nothing; the pencil enters edit mode, taps stage the same cycle locally (the card previews its state and streak), and a Save pill in the header commits the batch. Closing a sheet with staged changes asks "Discard changes?"; a clean sheet closes at once. After Save the sheet stays open in view mode and shows "Changes saved".
*Tech Notes*:
- **`lib/ui/widgets/day_details_modal.dart`** — `DayDetailsModal` is now a `ConsumerStatefulWidget` holding `_editing`, a `_staged` map (habit id → status the row will be saved as; a row cycled back to its persisted value is un-staged) and a `_stagedProgress` map (a quantitative habit's number); `isNotEmpty` on either is "dirty". `_isWithinEditWindow` became `_isQuickLogDay` (same today/yesterday predicate, re-evaluated on every tap). In edit mode a quantitative card opens `TargetEntrySheet` in DRAFT mode (`initialAmount` + `onChanged`): the sheet holds the number itself and reports each step, the card previews the amount, its derived verdict and streak, and Save writes it through `HabitProgressNotifier.setProgress`. On a quick-log day the sheet still commits live. A verified habit's "set by you" release is staged as a null status. The X and the barrier go through `PopScope` + `showEvolveConfirm`; drag-to-dismiss is DECLINED while dirty because a modal sheet's drag pops unconditionally (bottom_sheet.dart `onClosing` → `Navigator.pop`) and cannot be intercepted.
- **`lib/providers/goal_provider.dart`** — `nextManualStatus` is the one definition of the cycle (used by `cycleStatus` and the sheet). New `HabitLogsNotifier.setStatus(date, id, status)` sets a row directly through the same `_setManualStatus` path (persist, streak, D9 freeze), which now returns whether the write landed so a batched Save stops at the first failed row and keeps the rest staged. New `recomputeStreaksForHabits(ids)` rewrites the stored streak of every later row of the changed habits — a row stores the streak as of its own day and both analytics read `current_streak` from the latest row, so a single-day write cannot fix the rows after it.
- **`lib/core/private_data_store.dart` / `private_local_database.dart`** — `recomputeStreaksForGoals(ids)` runs the import's `recomputeStreaks` in one transaction with a fresh `updated_at` stamp (an unbumped correction loses LWW on the other devices).
- **`lib/core/backup_import_service.dart`** — the cloud recompute is now the public `recomputeCloudStreaks(client, ids)` (+ `bulkUpsertRows`), shared by the import and the notifier so the two cannot disagree about a row's streak.
- **`lib/core/verification_wiring.dart`** — `couldNotVerifyDaysProvider` and `manuallyResolvedDaysProvider` are no longer bounded to today/yesterday: the "?" shows on any past day up to today, and the freeze marker spans from the earliest rule's `effectiveFrom`. The sheet shows the marker only where the rule governed the day (`verificationRuleAppliesOn`).
- **`lib/ui/widgets/habit_calendar_widget.dart`** — `_isYesterdayOrToday` → `_isQuickLogDay`; the cell tint now means "quick-log day", not "the only editable days".
- **`lib/ui/widgets/target_entry_sheet.dart`** — now a `ConsumerStatefulWidget` with the optional draft mode above; without `onChanged` it behaves exactly as before.
- **i18n** — `habits.youCanOnlyEditTodayAnd` deleted; added `habits.editThisDay`, `tapEditToChangeThisDay`, `changesSaved`, `discardChangesTitle`, `discardChangesBody`, `discard`, `keepEditing` in all 5 source locales; `dart run slang` regenerated.
- **Tests** — `test/day_details_past_day_edit_test.dart` (the sheet over the real notifier and a fake private store: view-mode tap writes nothing, staging writes nothing, Save writes exactly the staged rows and recomputes exactly those habits, discard prompt, partial failure keeps the sheet in edit mode, today still writes on tap) and `test/habit_logs_set_status_test.dart` (`setStatus` semantics, rollback, recompute dispatch). `support/fake_private_data_store.dart` records `recomputeStreaksForGoals`.
- **Verification**: `flutter analyze` clean; `flutter test` 1149 passed, 0 failed (the two new files add 13). Not exercised on a device in this change: the drag-to-dismiss decline while dirty and the toast/haptic feel are widget-tested only.
...
**Five of six rejections are fixed in code** (1.5, 4.0, 3.1.2, 2.5.1, 3.1.1, 5.1.2(i)). **The sixth — 2.1, Screen Time — was to be answered by REPLYING to Apple, not by shipping**: for build 20 the truthful answer was "no, build 20 has no Screen Time functionality", and the feature was dark. _(Superseded 2026-07-27: Screen Time is no longer dark. `VerificationConfig.screenTimeAppsEnabled` is now **true** (`lib/core/verification_config.dart:29`) and `screenTimeEnabled` is `screenTimeAppsEnabled || screenTimeTotalEnabled` (:39-40), so Mode A — verification against a `FamilyActivityPicker`-picked app set — ships live, with its own always-visible Settings section. Only Mode B, `screenTimeTotalEnabled` (total device usage, which still rests on the unverified "empty selection = all activity" semantics), remains false. `TO_SIMO_DO.md` no longer carries a drafted 2.1 reply, and any answer to App Review must now describe the shipped Mode A rather than repeat the build-20 wording.)_
...
**Screen Time has since been picked up.** Mode A (`screenTimeAppsEnabled`) shipped: the user picks an app/category set with the native `FamilyActivityPicker` and the threshold measures that picked set as one DeviceActivity activity, so it does not rest on the empty-set question. That question — whether a `DeviceActivityEvent` with empty application/category/webDomain sets fires at all — now gates only Mode B (`screenTimeTotalEnabled`, total device usage), which stays dark until it is answered on hardware.
...
- [2026-07-30]: Silenced the ~260 third-party pod warnings in the iOS build/archive log
  - *Details*: Building and archiving iOS succeeded but emitted ~260 warnings, all of them from vendored dependencies: 7 from `permission_handler_apple`'s `PhonePermissionStrategy.m` (deprecated `CTCarrier`) and the rest from SQLCipher's `sqlite3.c` amalgamation. Applied the same fix the macOS target already uses (`desktop/macos/Podfile`): `inhibit_all_warnings!` in the `target 'Runner'` block, plus `LastUpgradeCheck` bumps so Xcode 26.6 stops nagging about recommended settings on both projects. `Runner` and `DeviceActivityMonitorExtension` keep every warning — only Pods go quiet.
  - *Tech Notes*:
    - **Files**: `ios/Podfile` (added `inhibit_all_warnings!`; `post_install` now sets `installer.pods_project.root_object.attributes['LastUpgradeCheck'] = '2660'`), `ios/Runner.xcodeproj/project.pbxproj` (`LastUpgradeCheck` 1510 → 2660; `LastSwiftUpdateCheck` was already 2660 and `CreatedOnToolsVersion = 26.6`, so the project attribute was simply stale). No dependency, Dart, or Swift changes.
    - **Why suppression and not a real fix — checked, not assumed.** `permission_handler_apple-9.4.7/ios/Classes/strategies/PhonePermissionStrategy.m:35-64` calls `CTCarrier`, `serviceSubscriberCellularProviders`, `subscriberCellularProvider` and `mobileNetworkCode`, deprecated in iOS 16 with no replacement. Three things rule out the alternatives: (1) **a version bump does not help** — upstream `main` (latest 9.4.10, published ~2026-06) still contains the identical unguarded code, and the changelog never mentions CoreTelephony; (2) **the file cannot be compiled out** — `PermissionHandlerEnums.h` defines a `PERMISSION_*` macro for 15 permissions but there is **no `PERMISSION_PHONE`**, and unlike `MediaLibraryPermissionStrategy.m` the phone strategy has no `#if` guard, so it is always compiled even though the app only ever asks for `Permission.notification` (`lib/ui/screens/consent_screen.dart:39,48` — the other three `permission_handler` importers only call `openAppSettings()`); (3) **patching in place is not durable** — the source lives in `~/.pub-cache`, so the next `pub get` reverts it. Same story for SQLCipher: `sqlite3.c` is generated code and `sqflite_sqlcipher`'s podspec pins `SQLCipher` to exactly `4.10.0`, so there is no newer pod to move to. The sqlite3.c noise is i64→int narrowing, ambiguous `MIN`/`MAX` (its own macros vs `sys/param.h`), two unreachable-code branches, and a `#warning` that `gethostuuid()` is unavailable on iOS — all benign, all in generated code.
    - **`inhibit_all_warnings!` covers the transitive pods too.** `SQLCipher` and `FMDB` are not in the Podfile — they arrive through `sqflite_sqlcipher`'s podspec. CocoaPods resolves `inhibit_warnings` per pod target with `inhibit_all_warnings?` short-circuiting to true, so transitive pods are included; the macOS target is the empirical proof, where this same line was added specifically to kill "SQLCipher ~100+ implicit conversion warnings" from the identical dependency chain.
    - *Verification*: **No Xcode and no CocoaPods on this machine** (`xcode-select -p` → CommandLineTools; `pod` not installed), so the archive log itself is unverified — that check is Simone's, and it is in `TO_SIMO_DO.md`. What was verified here: `ruby -c ios/Podfile` → Syntax OK; `plutil -lint ios/Runner.xcodeproj/project.pbxproj` → OK (the pbxproj still parses after the edit); and the real Podfile was evaluated against a stubbed CocoaPods DSL with a fake 7-pod installer, asserting `inhibit_all_warnings!` fires inside `target 'Runner'`, all 7 targets still reach `flutter_additional_ios_build_settings`, all 21 configs still get `IPHONEOS_DEPLOYMENT_TARGET = 16.0`, and the Pods project ends at `LastUpgradeCheck = 2660`. **Mutation-verified**: the same harness run against the pre-fix Podfile fails on exactly the two assertions the change adds.

---

## [2026-07-30] App Review remediation — Guidelines 3.1.2(c) and 5.1.1(v) (iOS)

*Details*: iOS 1.1.2 build 23 was rejected on 2026-07-21 (submission `70f9af73-8af2-4680-af51-756a2937d273`) on two guidelines. Both are addressed here. macOS is deliberately untouched — it was being worked on in parallel.

**3.1.2(c) — missing Terms of Use (EULA) link.** Root-caused, not patched. The App Store description was machine-translated as one string with the legal block at the *end* of the text and no length guard, so translation expansion pushed five localisations past App Store Connect's 4000-character ceiling and the truncation ate exactly the required links. Confirmed live, not inferred: `itunes.apple.com/lookup` returned a 3861-char German description ending in a literal `....` with no EULA and no privacy link, matching `mobile/metadata/de-DE/description.txt` byte-for-byte. French was identical; Mexican Spanish had a truncated privacy URL.

Two rival metadata trees existed (`mobile/metadata`, 47 entries with the full field set, and `mobile/ios/fastlane/metadata`, 39 description-only entries on older copy, six of them already over 4000 chars). Copy was being moved between them by hand, which is how the truncation happened. The second tree is deleted.

**5.1.1(v) — registration required to purchase.** The finding was factually wrong — Private mode force-injects `isPro: true` and has always unlocked every on-device Pro feature for free with no account — but the app made it unreadable. The first screen was a login wall with the no-account path fifth, in secondary chrome, labelled "Continue privately on this iPhone" (a sync preference, not "skip registration"), and the review notes steered the reviewer straight to a test account without mentioning Private mode at all.

*Tech Notes*:
- **New**: `mobile/tool/appstore/{locales.json,build_metadata.py,validate_metadata.py}` and `mobile/metadata_src/descriptions/*.txt` (39 translated bodies, no legal block). The body is translated; the legal block is *composed after* translation from per-locale labels plus hardcoded URLs, so a translator never sees a URL and the block can never sit in the truncatable tail. Each locale's body budget is `4000 − len(its own block) − 3`; an over-budget body is a hard build error, never a silent trim. `privacy_url.txt` and `support_url.txt` are emitted from the same `site` mapping so they cannot drift from the links inside the description.
- All 39 descriptions regenerated. 17 were still on pre-rewrite copy naming Supabase and Apple Sign-In as the only data story — which contradicted the 5.1.1 defence. The hardcoded "EUR 4.99 / EUR 49.99" was removed from the 22 descriptions that stated it (it was quoting euros to Japanese, Thai and Brazilian readers; the paywall reads live StoreKit prices anyway).
- **New CI job** `appstore-metadata`: rebuilds from source, fails on any diff against the committed tree, then validates length, both required URLs, and truncation-ellipsis signatures. New Fastfile lanes `validate_metadata` and `upload_metadata` (explicit `metadata_path`, validates first, never auto-submits). Deleted the broken `translate_descriptions.py` — it pointed at `/Users/simo/...`, a path that does not exist, and had no length guard.
- **New screen** `lib/ui/screens/data_mode_choice_screen.dart` at route `/choose`, between consent and login. Two equal cards, "Continue without an account" first, no credential field anywhere, and a footnote carrying Apple's own suggested wording that signing in is optional and exists for cross-device subscription access. `main.dart` redirects signed-out users to `/choose`; `/login` stays reachable (pushed, so back returns to the chooser).
- Auth screen's private path relabelled to "Continue without an account".
- **Legal reachability**: legal links previously existed *only* on the consent screen (first launch), the auth screen (signed out) and the paywall (`if (!isPro)`, hidden entirely in Private mode). A Private-mode user could reach no legal document after onboarding and a subscriber could not reach the EULA at all. Added a Settings › Legal section (Privacy, Terms, Apple EULA, Manage subscription) visible in every mode; added the links to the `isPro` branch; the Subscription row is no longer hidden in Private mode and opens a screen stating every Pro feature is already included at no cost. `initState` skips RevenueCat in Private mode — it is never configured there, so the screen would otherwise hang on its spinner.
- Removed `NSLocationWhenInUseUsageDescription` from `Info.plist` and all five `.lproj/InfoPlist.strings`: no location package exists in `pubspec.yaml`, and the string justified itself as "third-party integrations, even though it does not actively track it".
- New strings in all 5 in-app languages (en/it/es/de/ar): `settings.legal.*`, `auth.chooser*`, `auth.continueWithoutAccount*`, `auth.signInToSync*`, `subscription.private*`.
- Review notes rewritten to lead with the no-account path and the exact tap sequence.
- **Verification**: `flutter analyze --fatal-infos --fatal-warnings` clean; 680 tests pass (10 new across `data_mode_choice_screen_test.dart`, `private_mode_subscription_screen_test.dart`, `settings_legal_section_test.dart`); metadata validator green on all 39 locales.

**Current Status**: code complete and verified locally. The App Store itself is still broken until the metadata is uploaded — see `TO_SIMO_DO.md` for the seven manual steps, of which uploading and then re-checking the German/French storefronts are the blocking ones.
