...
*Tech Notes*:
- **`lib/ui/widgets/day_details_modal.dart`** — `DayDetailsModal` is now a `ConsumerStatefulWidget` holding `_editing` and a `_staged` map (habit id → status the row will be saved as; a row cycled back to its persisted value is un-staged, so `isNotEmpty` is "dirty"). `_isWithinEditWindow` became `_isQuickLogDay` (same today/yesterday predicate, re-evaluated on every tap). Quantitative habits open `TargetEntrySheet` for the day in edit mode and commit on their own; a verified habit's "set by you" release is staged as a null status. The X and the barrier go through `PopScope` + `showEvolveConfirm`; drag-to-dismiss is DECLINED while dirty because a modal sheet's drag pops unconditionally (bottom_sheet.dart `onClosing` → `Navigator.pop`) and cannot be intercepted.
