...
_(Superseded 2026-07-27: fixed. `clearAllOnDuplicateFailure` and the `storage.deleteAll()` fallback are gone; `secure_storage_utils.dart:162-168` scopes duplicate recovery to the single offending key — delete-then-rewrite just that key — and carries a comment naming SEC-6 as the reason never to widen it.)_
...
_(Superseded 2026-07-27: resolved. All nine objects listed below now live in `migrations/` — `20260622_add_{get_habit_analytics,get_global_trend,get_critical_habits,get_best_habits,get_habit_performance_by_day,get_habit_alerts,get_habit_yearly_grid,get_habit_correlations,habit_stats_view}.sql` — so a fresh project built from `migrations/` + `schema.sql` no longer breaks statistics. The recommended guard landed too: `mobile/test/schema_drift_test.dart` fails CI if `mobile/lib`, `desktop/lib` or any `packages/*/lib` reaches for a relation or RPC the SQL sources don't define.)_
...
_(Superseded 2026-07-27: the measurements and the CI recommendation are both out of date. `mobile/test/` is now 82 files / ~14,100 LOC against ~69,700 LOC in `mobile/lib`, and the "No tests for" list below is stale — auth flows (`auth_state_test.dart`), `goal_provider` sync/write logic (`goal_write_no_cascade_test.dart`, `cloud_import_plan_test.dart`, plus the `packages/evolve_sync` suite incl. `sync_engine_test.dart`), streak math (`streak_utils_test.dart` and its DST/frequency variants), the Private DB (`private_db_recovery_test.dart`, `private_db_schema_test.dart`), notifications (`notification_skip_test.dart`) and OpenRouter parsing (`openrouter_stream_test.dart`) all have suites now. CI landed too: `.github/workflows/mobile-ci.yml` ("Flutter CI") runs `flutter analyze --fatal-infos --fatal-warnings` + `flutter test` over mobile, desktop and the four `packages/`. What remains true is that `test/widget_test.dart` never pumps the widget.)_
