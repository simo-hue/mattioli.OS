...
## Bug Fixes
- [ ] **Aggiornare la funzione trigger `handle_new_user` su Supabase**:
  - Il login con Google falliva con errore database perché Google non fornisce `sentry_consent` nei metadati, e il trigger cercava di inserire NULL in una colonna NOT NULL.
  - Esegui questo SQL nell'editor di Supabase per aggiornare la funzione:
    ```sql
