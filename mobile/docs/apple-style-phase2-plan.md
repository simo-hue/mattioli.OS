...
## Resume instructions
**Nothing to resume — this roadmap finished on 2026-07-11** (see "✅ ALL COMPLETE" above). The unticked checkboxes in the Task #8–#16 lists and the "REMAINING T14" note record the state part-way through the effort: "Post-QA continuation" closes the T14 remainder, and "MOBILE COMPLETE" closes the rest. The only items never migrated are the two called out there as optional — the `subscription_screen` Pro-detail rows and the pre-existing privacy import Replace/Merge `RadioListTile`. Kept for reference: the kit primitives all live in `lib/ui/kit/` (see the primitive table), and the reference patterns are `category_picker_sheet.dart` (selection sheet) and `statistics_screen._showGoalSelector` (Pro-lock sheet).
