...
/// Reads the `profiles` row for [owner], or an EMPTY map when that owner has
/// none.
///
/// An empty map, and never a seeded one. This used to call `_ensureProfile` and
/// recurse, so a READ minted a whole identity — a `profiles` row AND the
/// `goal_category_settings` row that goes with it — for whatever id `ownerId()`
/// happened to return. [_open] already refuses to do that, and says why: it
/// reconciles the owner id BEFORE seeding because "a stale owner id gets a full
/// identity minted for it ... that is how a user ended up with 3 profiles".
///
/// The window is real. [SyncEngine.enable] calls [adoptOwner] before
/// `reKeyOwner` moves the rows, so a re-key that fails (SQLITE_BUSY from the
/// notification isolate, an app kill) leaves `ownerId()` answering the
/// canonical id while every row still carries the old one — and any of the
/// several read paths through here would then mint the pair. Because
/// `goal_category_settings.user_id` is UNIQUE, that shell makes every later
/// re-key fail on the constraint: iCloud sync wedged permanently, by a read.
///
/// Callers already tolerate the empty map (they read individual keys with
/// null-safe defaults), and [_open] guarantees the pair exists for the owner it
/// reconciled — which is the owner every read here is asking about in normal
/// operation.
///
/// Split out to take a [DatabaseExecutor] for the same reason as
/// [readExistingHabitLog]: the database is opened through SQLCipher, whose
/// native plugin does not exist in the Flutter test VM, so the method itself
/// cannot be driven from a test.
Future<Map<String, dynamic>> readProfileRow(
  DatabaseExecutor db,
  String owner,
) async {
...
/// Mirrors the cloud `get_macro_goals_stats` RPC for Private mode; [year] is
/// 'all' or a specific year string.
///
/// PUBLIC and top-level so it can be exercised directly. It used to be inline
/// in [PrivateLocalDatabase.macroGoalsStats], whose database is opened through
/// SQLCipher — a plugin that does not exist in the Flutter test VM — so nothing
/// could pin it, and it had silently drifted from the RPC it mirrors (the
/// 'most productive year' ranked on COMPLETED instead of total, and the -1 loop
/// seeds escaped as real values the UI printed). Same split, and the same
/// reason, as [readExistingHabitLog] and [goalToRow].
Map<String, dynamic> computeMacroGoalsStats(
  List<MacroGoal> allGoals,
  String year,
) {
  if (year == 'all') {
    ...
    for (final y in sortedYears) {
      ...
      // Ranked on the year's TOTAL, like the RPC's most_prod_year_stat
      // (`ORDER BY total DESC LIMIT 1`, returning that total) — and like the
      // card's own subtitle, which calls the number "total goals".
      if (t > mostProdCount) {
        mostProdCount = t;
        mostProdYear = y;
      }
    ...
    return {
      ...
      'best_year': bestYear,
      // The loop seeds are sentinels, not answers: with no year bucket the RPC's
      // LIMIT-1 CTEs yield SQL NULL, and the view's `as int? ?? 0` cannot fire
      // on a valid -1 — which is how "-1%" reached the screen.
      'best_year_rate': bestYearRate < 0 ? null : bestYearRate,
      'most_productive_year': mostProdYear,
      'most_productive_count': mostProdCount < 0 ? null : mostProdCount,
    ...
    return {
      ...
      // Sentinels, not answers — see the 'all' branch above.
      'best_category_rate': bestCategoryRate < 0 ? null : bestCategoryRate,
      'best_month': bestMonth,
      'best_month_rate': bestMonthRate < 0 ? null : bestMonthRate,
      'best_type': bestType,
      'best_type_rate': bestTypeRate < 0 ? null : bestTypeRate,
...
class PrivateLocalDatabase implements PrivateDataStore {
  ...
  Future<Map<String, dynamic>> loadProfileRow() async =>
      readProfileRow(await _database(), await ownerId());
  ...
  Future<Map<String, dynamic>> macroGoalsStats(String year) async =>
      computeMacroGoalsStats(await loadMacroGoals(), year);
