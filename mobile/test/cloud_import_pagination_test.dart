// Regression tests for finding #39: the Cloud (Supabase) import's existing-state
// reads must page past PostgREST's per-request `db-max-rows` cap. A single
// unbounded select silently returns only the first ~1000 rows, which makes
// "Replace" leave stale rows behind and makes the streak recompute run on a
// truncated history.
//
  ...
  return MockClient((req) async {
    ...
    if (req.method == 'GET') {
      ...
      if (windowed) {
        ...
        final limit = int.tryParse(params['limit'] ?? '$_dbMaxRows') ?? _dbMaxRows;
...
Future<SupabaseClient> _authedClient(MockClient mock) async {
  ...
  await client.auth.setInitialSession(jsonEncode({
    ...
    'token_type': 'bearer',
    'user': {'id': 'u1', 'app_metadata': <String, dynamic>{}, 'aud': 'authenticated'},
  }));
  return client;
}
...
void main() {
  ...
  test(
    ...
    final row =
        planUpsert.rows.firstWhere((r) => r['date'] == witnessDate);
