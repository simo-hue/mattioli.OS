...
/// DATA-4: Private Mode analytics must match the captured cloud RPC/view
/// semantics (see ../migrations/20260622_*). These fixtures are hand-computed
/// against those SQL definitions.
void main() {
  final today = DateTime(2026, 6, 15);
  DateTime d(int month, int day) => DateTime(2026, month, day);

  HabitLogEntry log(String goal, DateTime date, String status, [int streak = 0]) =>
  ...
  group('computeAnalyticsRow (get_habit_analytics)', () {
    test('worst_dow = lowest done-rate weekday', () {
      ...
      final tuesday = d(6, 16);
      final row = computeAnalyticsRow(goalId: 'g1', logs: [
        log('g1', mondayA, 'done'),
  ...
  group('computeGlobalTrend (get_global_trend)', () {
    GoalInput alwaysActive(String id) =>
        GoalInput(id: id, startDate: d(1, 1));
    ...
    test('week: 14 daily points, indices 0..13, dates today-13..today', () {
      ...
      expect(points.first['date'], dateKey(today.subtract(const Duration(days: 13))));
    ...
    test('year: 24 monthly points ending on the current month', () {
      ...
      expect(points.last['date'], dateKey(DateTime(today.year, today.month, 1)));
    });

    test('all: buckets from earliest start to today', () {
      final points = computeGlobalTrend(
        goals: [GoalInput(id: 'g1', startDate: today.subtract(const Duration(days: 5)))],
        logs: const {},
